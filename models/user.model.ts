...
class User {
    ...
    static create = (username: string) => {
        sql.query(
            `CREATE TABLE IF NOT EXISTS ${username} (id INT AUTO_INCREMENT PRIMARY KEY, cryptoCurrency VARCHAR(50), 
            UNIQUE (cryptoCurrency)) ENGINE=InnoDB DEFAULT CHARSET=utf8;`,
            (err) => {
                if (err) {
